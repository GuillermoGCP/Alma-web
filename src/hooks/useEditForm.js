  ...
  const onSubmit = async (data, jsonNumber) => {
    try {
      ...
      if (updateResponse.ok) {
        ...
        setForms((prevData) => {
          const updatedData = Object.entries(prevData).map(([key, value]) => {
            if (key === Object.keys(dataFromBack.apdatedForm)[0]) {
        ...
        if (checkPublishedResponse.ok) {
          ...
          if (publishedData.isPublished) {
            try {
              ...
              if (publishResponse.ok) {
                const publishedFormData = await publishResponse.json()
          ...
          console.error('Ha habido un error al verificar si está publicado')
        }
      } else {
